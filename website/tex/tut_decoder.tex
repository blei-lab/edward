...
based on coding theory, where $p(\mathbf{x}_n\mid \mathbf{z}_n)$ defines
a distribution over each value
$\mathbf{x}_n\in\mathbb{R}^D$ given a code $\mathbf{z}_n$. The latent
variables are interpreted as the hidden representation, or code, of the value
$\mathbf{x}_n$. The decoder is probabilistic because its decoding for
any given code is random.
...
Let's define the model in Edward using TensorFlow, and with
PrettyTensor as an easy way to build neural networks. Here we use a
probabilistic decoder to model 28 x 28
pixels in a binarized MNIST data set.
