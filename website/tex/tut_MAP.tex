...
\end{align*}
where the last expression can be thought of as an approximation to the cross
entropy between the true data distribution and the model $p(x\mid z)$,
...
The \texttt{MAP} class automatically initializes a point mass
variational distribution. The \texttt{build_loss} method implicitly specifies the gradient
of the log joint. TensorFlow's automatic differentiation will apply to \texttt
{z}, thus backpropagating (applying the chain rule of differentiation)
