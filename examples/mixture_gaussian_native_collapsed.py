...
temp = Normal([mu, sigma],
              lambda cond_set: tf.pack([cond_set[0] for n in range(N)]),
              lambda cond_set: tf.pack([cond_set[1] for n in range(N)]))
# TODO i can't do this; i need to create a DelayedTensor object which
# enables delayed deterministic computations on top of a distribution
# tensor
x = tf.reduce_sum(temp, 1)
