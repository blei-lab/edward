...
class MixtureGaussian:
    ...
    p(x, z) = [ prod_{n=1}^N N(x_n; mu_{c_n}, sigma_{c_n}) Multinomial(c_n; pi) ]
              [ prod_{k=1}^K N(mu_k; 0, cI) ]
              Dirichlet(pi; alpha)

    where z = {pi, mu} and for known hyperparameters a, b, c, sigma, alpha.
...
class MFMixGaussian(Likelihood):
    ...
    def sample(self, size, sess):
        ...
        z = np.concatenate((z_dirichlet, z_gaussian), axis=1)
...
inference.run(n_iter=1000, n_minibatch=5)
