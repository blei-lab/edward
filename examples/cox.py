...
"""Cox process model using mean-field variational inference.
   Ref: Cox, 1955 and Miller et al, 2014.
"""
...
class CoxProcess:
  ...
  def kernel_row(self, x):
    ...
    mat = tf.pack(mat)
    #ensure this is positive definite
    mat = mat + tf.diag(
          tf.convert_to_tensor([1e-6, 1e-6], dtype=tf.float32))
  ...
  def log_prob(self, xs, zs):
    ...
    chol = tf.cholesky(self.kernel(x))

    log_prior = multivariate_normal_cholesky.logpdf(
      z, tf.zeros([self.N, self.V]), chol)
