...
class VariationalInference(Inference):
    ...
    def initialize(self, n_iter=1000, n_data=None, n_print=10):
        """
...
class ChiVI(VariationalInference):
    ...
    """
    def __init__(self, model, variational, data=Data(), n=10.0):
        VariationalInference.__init__(self, model, variational, data)
        self.n = n
    ...
    def build_score_loss(self):
        ...
        # gradient(exp(n*CUBO))
        # = (1-n) * E_{q(z; lambda)} [ grad log q(z; lambda) * w(z; lambda)^n ]
        return -(1-self.n) * tf.reduce_mean(log_omega * tf.stop_gradient(self.losses))
    ...
    def build_reparam_loss(self):
        ...
        for i in range(self.variational.num_vars):
            q_log_prob += self.variational.log_prob_zi(i, z)
        #print("model.log_prob",sess.run(self.model.log_prob(x, z)))
        
        log_omega = self.model.log_prob(x, z) - q_log_prob 
        self.losses = tf.pow(tf.exp(log_omega), self.n)
        # TODO this doesn't work although it should
        #return -tf.reduce_mean(self.losses)
        #sess.close()
    ...
    def print_progress(self, t, losses, sess):
        if t % self.n_print == 0:
            ...
            print("iter %d exp(n*cubo) %.2f " % (t, loss))
            self.variational.print_params(sess)
