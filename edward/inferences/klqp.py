...
def build_reparam_loss_and_gradients(inference, var_list):
  ...
  for s in range(inference.n_samples):
    ...
    for z, qz in six.iteritems(inference.latent_vars):
      ...
      q_log_prob[s] += inference.scale.get(z, 1.0) * \
          tf.reduce_sum(qz_copy.log_prob(z_sample[z]))
    ...
    if inference.model_wrapper is None:
      ...
      for z in six.iterkeys(inference.latent_vars):
        ...
        p_log_prob[s] += inference.scale.get(z, 1.0) *  \
            tf.reduce_sum(z_copy.log_prob(dict_swap[z]))

      for x in six.iterkeys(inference.data):
        if isinstance(x, RandomVariable):
          x_copy = copy(x, dict_swap, scope=scope)
          p_log_prob[s] += inference.scale.get(x, 1.0) * \
  ...
  for s in range(inference.n_samples):
    ...
    if inference.model_wrapper is None:
      ...
      for x in six.iterkeys(inference.data):
        if isinstance(x, RandomVariable):
          x_copy = copy(x, dict_swap, scope=scope)
          p_log_lik[s] += inference.scale.get(x, 1.0) * \
              tf.reduce_sum(x_copy.log_prob(dict_swap[x]))
    else:
      x = inference.data
...
def build_score_loss_and_gradients(inference, var_list):
  ...
  for s in range(inference.n_samples):
    ...
    for z, qz in six.iteritems(inference.latent_vars):
      ...
      z_sample[z] = qz_copy.value()
      q_log_prob[s] += inference.scale.get(z, 1.0) * \
          tf.reduce_sum(qz_copy.log_prob(tf.stop_gradient(z_sample[z])))
