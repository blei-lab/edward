...
def compute_multinomial_mode(probs, total_count):
    ...
    ```
    """
    remaining_count = tf.constant(total_count)
    mode = tf.zeros_like(probs)
    uniform_prob = tf.get_variable('uniform_prob', initializer=1/total_count)
    zeros = tf.get_variable('zeros', initializer=tf.zeros_like(probs))
    tf.variables_initializer([uniform_prob, zeros]).run()

    # mode, probs, remaining_count = _multinomial_mode_body(mode, probs, remaining_count)

    mode, _, _ = tf.while_loop(
        _multinomial_mode_cond,
        _multinomial_mode_body,
        loop_vars=[mode, probs, remaining_count],
    )
    return mode


def _multinomial_mode_cond(mode, probs, remaining_count):
    # return tf.not_equal(remaining_count, tf.constant(0))
    return tf.greater(remaining_count, tf.constant(0))


def _multinomial_mode_body(mode, probs, remaining_count):
    with tf.variable_scope('', reuse=True):
        uniform_prob = tf.get_variable('uniform_prob')
        zeros = tf.get_variable('zeros')
        probs = tf.cond(tf.reduce_all(probs < uniform_prob),
            lambda: tf.nn.softmax(probs),
            lambda: probs
        )

        indices = _compute_assignment_indices(probs, remaining_count)
        n_updates = tf.size(indices)
        update_shape = (n_updates,)
        mode_updates = tf.ones(update_shape)
        probs_updates = -uniform_prob * mode_updates

        mode += tf.scatter_add(zeros, indices, mode_updates)
        probs += tf.scatter_add(zeros, indices, probs_updates)
        remaining_count -= n_updates
        return mode, probs, remaining_count


def _compute_assignment_indices(probs, remaining_count):
    with tf.variable_scope('', reuse=True):
        uniform_prob = tf.get_variable('uniform_prob')
        indices = tf.squeeze(tf.where(probs >= uniform_prob))
        overflow_count = tf.size(indices) - remaining_count
        cond = tf.logical_and(
            tf.greater_equal(overflow_count, tf.constant(1)),
            tf.less_equal(overflow_count, tf.size(probs))
        )
        return tf.cond(cond,
            lambda: _get_n_indices(indices, remaining_count),
            lambda: indices
        )


def _get_n_indices(indices, n):
    uniform_sample = tf.random_uniform(shape=(1, tf.size(indices)))
    top_k_indices = tf.squeeze(tf.nn.top_k(uniform_sample, k=n).indices)
    return tf.gather(indices, top_k_indices)
