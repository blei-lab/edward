...
pi = Dirichlet(tf.ones(K))
mu = Normal(mu=tf.zeros(D), sigma=tf.ones(D), sample_shape=K)
sigma = InverseGamma(alpha=tf.ones(D), beta=tf.ones(D), sample_shape=K)
cat = Categorical(p=pi, sample_shape=N)
components = [
    MultivariateNormalDiag(mu=mu[k], diag_stdev=sigma[k], sample_shape=N)
    for k in range(K)]
